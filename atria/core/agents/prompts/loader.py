...
def load_tool_description_from_path(path: Path) -> str:
    """Load a tool description directly from an absolute markdown path.

    Used by skill-owned tools whose description cards live inside the skill
    folder rather than the built-in templates directory.
    """
    p = Path(path)
