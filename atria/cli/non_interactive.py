"""Non-interactive mode handler for Atria CLI."""
...
from atria.models.agent_deps import AgentDependencies
...
def _run_non_interactive(
    ...
    if not session:
        session = run_sync(
            session_manager.create_session(working_directory=str(config_manager.working_dir))
        )
    ...
    run_sync(session_manager.add_message(user_msg, config.auto_save_interval))
    ...
    run_sync(session_manager.add_message(assistant_msg, config.auto_save_interval))
    run_sync(session_manager.save_session())
