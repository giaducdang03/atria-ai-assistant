"""Main entry point and argument parsing for Atria CLI."""
...
from atria.ui_textual.style_tokens import ERROR, WARNING
from atria.setup import run_setup_wizard
from atria.setup.wizard import config_exists
from atria.ui_textual.runner import launch_textual_cli
...
def _pick_session_interactively(working_dir: "Path") -> "str | None":
    ...
    sm = PgSessionManager(
        sessionmaker=_sm, working_directory=str(working_dir) if working_dir else None
    )

    sessions = run_sync(sm.list_all_sessions())
    if not sessions:
...
def main() -> None:
    """Main entry point for Atria CLI."""
    import sys
    ...
    parser = argparse.ArgumentParser(
        prog="atria",
        description="Atria - AI-powered command-line tool for accelerated development",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  atria                          # Start interactive CLI session
  atria "do something"           # Start session with initial message
  atria run ui                   # Start web UI (backend + frontend) and open browser
  atria -p "create hello.py"     # Non-interactive mode
  atria mcp list                 # List MCP servers
  atria mcp add myserver uvx mcp-server-example
        """,
    )
    ...
    config_parser = subparsers.add_parser(
        "config",
        help="Manage Atria configuration",
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description="Manage MCP servers for extending Atria with external tools and capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  atria mcp list                                    # List all servers
  atria mcp add myserver uvx mcp-server-sqlite      # Add SQLite MCP server
  atria mcp add custom node server.js arg1 arg2     # Add custom server with args
  atria mcp add api python api.py --env API_KEY=xyz # Add with environment variable
  atria mcp get myserver                            # Show server details
  atria mcp enable myserver                         # Enable a server
  atria mcp remove myserver                         # Remove a server
        """,
    )
    ...
    mcp_add = mcp_subparsers.add_parser(
        ...
        description="Register a new MCP server that will be available to Atria",
    )
    ...
    mcp_add.add_argument(
        "--no-auto-start",
        action="store_true",
        help="Don't automatically start this server when Atria launches",
    )
    ...
    # Support bare positional prompt: atria "hello" opens interactive TUI with initial message
    ...
    if args.command == "config":
        from atria.cli.config_commands import _handle_config_command
    ...
    # Handle MCP commands
    if args.command == "mcp":
        from atria.cli.mcp_commands import _handle_mcp_command
    ...
    # Handle run commands
    if args.command == "run":
        from atria.cli.run_commands import _handle_run_command
    ...
    try:
        ...
        from atria.core.context_engineering.history.session_manager import PgSessionManager
        from atria.db.connection import get_sessionmaker
        from atria.db.sync import run_sync as _run_sync
        ...
        env_session_dir = os.environ.get("ATRIA_SESSION_DIR")
        _sm = _run_sync(get_sessionmaker())
        ...
        if args.prompt:
            ...
            if config.verbose:
                from atria.core.debug import SessionDebugLogger, set_debug_logger

                session = _run_sync(session_manager.get_current_session())
                if session:
            ...
            if config.verbose:
                from atria.core.debug import get_debug_logger, set_debug_logger as _set_dl
        ...
        if args.resume is True:
            ...
            sessions = _run_sync(session_manager.list_all_sessions())
