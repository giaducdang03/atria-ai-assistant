...
from atria.core.agents.components import extract_plan_from_response
from atria.core.runtime import ConfigManager, OperationMode
...
from atria.ui_textual.managers.approval_manager import ChatApprovalManager
from atria.ui_textual.chat_app import create_chat_app
from atria.ui_textual.runner_components import (
    HistoryHydrator,
    ToolRenderer,
    ModelConfigManager,
    CommandRouter,
...
class TextualRunner:
    ...
    def _setup_runtime(
        ...
        if repl is not None:
            ...
            if session_manager is not None:
                ...
                self.session_manager = PgSessionManager(
                    sessionmaker=_sm,
    ...
    def _setup_app(self) -> None:
        ...
        model_display = normal_info.get("model_display", self.config.model)
        ...
        self._web_port = None
        if not os.environ.get("ATRIA_NO_WEB_BRIDGE"):
            try:
                import threading
                from atria.web.server import start_server
                ...
                from atria.web.logging_config import suppress_console_output
                from atria.core.paths import get_paths
                ...
                if port is not None:
                    ...
                    if user_store is None:
                        ...
                        user_store = PgUserStore(run_sync(get_sessionmaker()))
                    ...
                    from atria.web.bridge_guard import BridgeSessionGuard
        ...
        def _on_ready_with_hydration() -> None:
            ...
            from atria.ui_textual.managers.display_ledger import DisplayLedger
            ...
            if self._is_resumed_session and hasattr(self.app, "status_bar"):
                ...
                if session:
                    ...
                    if session_title:
                        set_terminal_title(f"Atria - {session_title}")
            ...
            from atria.ui_textual.managers.background_task_status import (
    ...
    def _run_query(self, message: str) -> list[ChatMessage]:
        ...
        try:
            ...
            try:
                ...
                from atria.ui_textual.chat_app import ConversationLog
            ...
            if conversation_widget is not None:
                ...
                from atria.ui_textual.ui_callback import TextualUICallback
                ...
                if getattr(self, "_web_server_thread", None) and self._web_server_thread.is_alive():
                    try:
                        from atria.ui_textual.bridge_callback import BridgeUICallback
            ...
            if _web_bridge_active:
                _s = run_sync(self.session_manager.get_current_session())
                self._web_broadcast_sync(
                    {
                        "type": "session_activity",
                        "data": {
                            "session_id": _s.id if _s else "unknown",
                            "status": "running",
                        },
                    }
                )
            ...
            if _web_bridge_active and react_executor is not None:
                try:
                    ...
                    from atria.web.state import get_state as _get_bridge_state

                    _bstate = _get_bridge_state()
                    _bloop = _bstate.get_event_loop()
                    _bws = _bstate.ws_manager
                    _bsession = run_sync(self.session_manager.get_current_session())
            ...
            try:
                ...
                if _web_bridge_active:
                    ...
                    cur_session = run_sync(self.session_manager.get_current_session())
...
"""
    ...
    def run(self) -> None:
        ...
        try:
            ...
            from atria.core.debug import get_debug_logger, set_debug_logger
...
def launch_textual_cli(
    ...
    **kwargs,
    ...
    if "auto_connect_mcp" not in kwargs:
        auto_env = os.getenv("ATRIA_MCP_AUTOCONNECT", "").strip().lower()
