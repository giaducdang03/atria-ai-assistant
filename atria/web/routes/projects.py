...
# ── Personal conversations (no project) ───────────────────────────────────────
# Separate router to avoid FastAPI matching "personal" as /{project_id}.
...
async def list_personal_conversations(
    ...
    sm = await get_sessionmaker()
    conv_repo = ConversationRepository(sm)
    rows = await conv_repo.list_personal()
    return [
        {
            "id": str(row["id"]),
            "name": row["title"] or f"Conversation {row['id']}",
...
@personal_router.post("/conversations")
async def create_personal_conversation(
    ...
    name = request.name.strip() or "New Chat"
    home = _Path(_os.path.expanduser("~"))
    conv_dir = home / ".atria" / "personal" / slugify(name)
    ensure_path(conv_dir)

    # Anonymous user (id=0) has no DB row — pass None to avoid FK violation.
    db_user_id = user.id if user.id and user.id > 0 else None
...
@personal_router.delete("/conversations/{conversation_id}")
