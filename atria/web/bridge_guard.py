...
class BridgeSessionGuard:
    ...
    async def create_session(self, **kwargs: Any) -> Any:
        ...
        return await self._inner.get_current_session()

    async def load_session(self, session_id: str, **kwargs: Any) -> Any:
        """Only allow loading the bridge session; others return current."""
        if session_id == self._bridge_session_id:
            return await self._inner.load_session(session_id, **kwargs)
        logger.debug("BridgeSessionGuard: load_session(%s) blocked, returning current", session_id)
        return await self._inner.get_current_session()

    async def load_latest_session(self, **kwargs: Any) -> Any:
    ...
    async def delete_session(self, session_id: str, **kwargs: Any) -> Any:
        ...
        if session_id == self._bridge_session_id:
            ...
            return None
        return await self._inner.delete_session(session_id, **kwargs)
