...
def _fetch_models_dev(url: str = MODELS_DEV_URL, timeout: int = 10) -> Optional[Dict[str, Any]]:
    request = urllib.request.Request(
        url,
        headers={
            "User-Agent": os.getenv("ATRIA_HTTP_USER_AGENT", "swecli/unknown"),
        },
    )
    try:
...
def load_models_dev_catalog(
    *,
    ...
    Precedence:
        1. ATRIA_MODELS_DEV_PATH environment variable (JSON file path)
        2. Cached response under ~/.atria/cache/models.dev.json (if fresh)
    ...
    override_path = os.getenv("ATRIA_MODELS_DEV_PATH")
    if override_path:
        ...
        if override.exists():
            ...
            _LOG.warning("ATRIA_MODELS_DEV_PATH %s does not exist", override)
    ...
    if disable_fetch is None:
        disable_fetch = os.getenv("ATRIA_DISABLE_REMOTE_MODELS", "").lower() in {"1", "true", "yes"}
...
def sync_provider_cache(
    ...
    Cache location: ~/.atria/cache/providers/{provider_id}.json
    ...
    if os.getenv("ATRIA_DISABLE_REMOTE_MODELS", "").lower() in {"1", "true", "yes"}:
        return False

    # Support ATRIA_MODELS_DEV_PATH override
    override_path = os.getenv("ATRIA_MODELS_DEV_PATH")
    if override_path:
        override = Path(override_path).expanduser()
        if override.exists():
