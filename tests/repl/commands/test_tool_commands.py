...
def test_init_command(tool_commands, mock_repl, tmp_path):
    ...
    # Create a fake ATRIA.md to simulate success
    atria_path = tmp_path / "ATRIA.md"
    atria_path.write_text("# Test")

    with patch("atria.repl.commands.tool_commands.load_prompt") as mock_load:
...
def test_init_command_shows_summary_in_tui(mock_repl, tmp_path):
    ...
    def mock_run_sync(**kwargs):
        ...
        if call_count[0] == 1:
            ...
            return {"success": True, "content": "Created ATRIA.md with overview."}
    ...
    # Create ATRIA.md to simulate success
    atria_path = tmp_path / "ATRIA.md"
    atria_path.write_text("# Test")
    ...
    mock_ui_callback.on_assistant_message.assert_called_once_with("Created ATRIA.md with overview.")
