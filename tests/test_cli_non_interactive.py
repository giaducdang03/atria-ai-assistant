...
from atria.cli import _run_non_interactive
...
def _setup_config(tmp_path, monkeypatch):
    ...
    config.atria_dir = str(tmp_path / ".atria")
    config.session_dir = str(tmp_path / ".atria" / "sessions")
    config.log_dir = str(tmp_path / ".atria" / "logs")
