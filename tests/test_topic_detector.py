...
class TestTopicDetectorInit:
    ...
    @patch("atria.core.agents.components.api.configuration" ".create_http_client_for_provider")
...
class TestTopicDetection:
    ...
    def _make_detector(self) -> TopicDetector:
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "sk-test"}, clear=False):
            with patch(
                "atria.core.agents.components.api.configuration" ".create_http_client_for_provider"
            ) as mock_create:
                mock_create.return_value = MagicMock()
                config = AppConfig(model_provider="openai", model="gpt-4o")
                detector = TopicDetector(config)
