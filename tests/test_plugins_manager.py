...
from atria.core.plugins.models import (
    MarketplaceInfo,
    KnownMarketplaces,
    PluginMetadata,
    InstalledPlugin,
...
from atria.core.plugins.config import (
...
from atria.core.plugins.manager import (
    PluginManager,
    PluginManagerError,
    MarketplaceNotFoundError,
    PluginNotFoundError,
)
from atria.core.paths import reset_paths, ENV_ATRIA_DIR
...
class TestConfig:
    ...
    def test_load_known_marketplaces_empty(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv(ENV_ATRIA_DIR, str(tmp_path / ".atria"))
        reset_paths()
    ...
    def test_save_and_load_known_marketplaces(self, tmp_path, monkeypatch):
        ...
        swecli_dir = tmp_path / ".atria"
        monkeypatch.setenv(ENV_ATRIA_DIR, str(swecli_dir))
        reset_paths()
...
class TestPluginManager:
    ...
    def test_validate_marketplace_valid_swecli_dir(self, tmp_path):
        """Test validating marketplace with .atria/marketplace.json."""
        manager = PluginManager()

        marketplace_dir = tmp_path / "test-mp"
        (marketplace_dir / ".atria").mkdir(parents=True)
        (marketplace_dir / ".atria" / "marketplace.json").write_text(
            '{"name": "test", "plugins": []}'
        )
...
"""
    ...
    @patch("atria.core.plugins.manager.marketplace.subprocess.run")
