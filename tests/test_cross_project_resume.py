...
def test_list_all_sessions_finds_cross_project(tmp_path):
    ...
    with patch("atria.core.paths.get_paths", return_value=mock_paths):
...
def test_cli_pick_session_uses_list_all_sessions(tmp_path):
    ...
    from atria.cli.main import _pick_session_interactively
    ...
    with (
        patch(
            "atria.core.context_engineering.history.session_manager.SessionManager",
            return_value=mock_sm,
        ) as mock_cls,
        patch(
            "atria.core.context_engineering.history.SessionManager",
            mock_cls,
        ),
    ):
