...
from atria.repl.commands.skills_commands import SkillsCommands, load_skill_generator_prompt
...
class TestSkillsCommandsList:
    ...
    def test_list_skills_with_skill(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create a test skill
            skill_dir = tmpdir_path / ".atria" / "skills" / "test-skill"
...
class TestSkillsCommandsCreate:
    ...
    @patch("atria.repl.commands.skills_commands.Prompt.ask")
    @patch("atria.repl.commands.skills_commands.Confirm.ask")
    def test_create_skill(self, mock_confirm, mock_prompt):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("atria.repl.commands.skills_commands.get_paths") as mock_paths:
                paths = MagicMock()
...
class TestSkillsCommandsDelete:
    ...
    def test_delete_skill(self, mock_confirm):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create a test skill
            skill_dir = tmpdir_path / ".atria" / "skills" / "delete-me"
            skill_dir.mkdir(parents=True)
...
class TestSkillsCommandsFindSkill:
    ...
    def test_find_skill_dir_project(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            skill_dir = tmpdir_path / ".atria" / "skills" / "project-skill"
            skill_dir.mkdir(parents=True)
            (skill_dir / "SKILL.md").write_text("---\nname: project-skill\n---\n")
